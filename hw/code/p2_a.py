...
plt.xlabel("Time (t)")
plt.ylabel("u(t)")
plt.title("Fourier Series Signal")
plt.legend()
plt.grid()
plt.savefig(os.path.join(figs_dir, "p2_ground_truth.png"))
plt.show()
...
for p in p_values:
    ...
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(figs_dir, f"p2a_reconstruction_p_{p}.png"))
    plt.show()
...
plt.title("Compressed Sensing Reconstructions for Different p Values", fontsize=16)
plt.legend(fontsize=12, loc='upper right')
plt.grid(True, linestyle='--', alpha=0.6)

# Save the high-resolution plot
plt.savefig(os.path.join(figs_dir, "p2a_combined_reconstructions.png"), dpi=300, bbox_inches='tight')
